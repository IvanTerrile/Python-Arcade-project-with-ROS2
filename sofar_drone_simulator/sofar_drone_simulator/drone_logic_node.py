...
from sofar_drone_simulator_interface.srv import Vtol, Locations

class RobotLogic(Node):

    def __init__(self):
        super().__init__("drone_logic_node")
...
def main(args=None):
    ...
    drone_logic_node = RobotLogic()
    
    # Spinning thread to make sure callbacks are executed
    spin_thread = Thread(target=rclpy.spin, args=(drone_logic_node,))
    spin_thread.start()

    # Start logic node routine
    drone_logic_node.routine()

    # On shutdown..
    drone_logic_node.get_logger().info("Shutdown logic node...")
    drone_logic_node.destroy_node()
    rclpy.shutdown()
